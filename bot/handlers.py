...
# @dp.message_handler(lambda message: message.text in ["Посмотреть ИУП студента"], state='*')
# async def view_isp(message: types.Message):
#     await bot.send_message(message.from_user.id, "Пожалуйста, введите имя и фамилию студента:")
...
#
# @dp.message_handler(
#     lambda message: message.text in [translations[lang]['report_of_grades'] for lang in translations.keys()], state='*')
# async def report_of_grades(message: types.Message):
#     language = user_sessions[message.from_user.id]['language']
#     await bot.send_message(message.from_user.id, translations[language]['choose_report_type'],
#                            reply_markup=report_types_markup(language))
...
async def grades_menu_handler(message: types.Message, state: FSMContext):
    language = user_sessions[message.from_user.id]['language']
    # Обработка выбора в меню "Отчет об оценке"
    if message.text == translations[language]['first_assessment']:
        # Обработка для "1 аттестация"
        await bot.send_message(message.from_user.id, "Обработка для 1 аттестация")
    elif message.text == translations[language]['second_assessment']:
        # Обработка для "2 аттестация"
        await bot.send_message(message.from_user.id, "Обработка для 2 аттестация")
    elif message.text == translations[language]['semester']:
        # Обработка для "Семестровый"
        await bot.send_message(message.from_user.id, "Обработка для Семестровый")
